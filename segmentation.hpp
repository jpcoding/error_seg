...
private:
  ...
  */
  ...
  void segmentation2D_2(const T* data, T threshold, T background)
  {
    int dim0 = global_dimensions[0];
    int dim1 = global_dimensions[1]; // fastest changing dimension is the last dimension
    ...
    for (int i = 0; i < dim0; i++)
    {
      for (int j = 0; j < dim1; j++)
      {
        // if a point is within -background and background, it is considered as background

        // positive will be one section
        if(data[i * dim1 + j] > 0)
        {
          if (j < dim1 - 1 &&
            std::abs(data[i * dim1 + j] -
